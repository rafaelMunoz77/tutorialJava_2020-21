...
public class EjemploExcepcionesBasico {
	...
	private static void ejemploNullPointerException () {
		...
		try {
			...
			System.out.println("cadena: " + cadenas[0]);
			
			int num1 = 0;
			int resultado = 56 / num1;
		}
	...
	private static void ejemploIndexOutOfBoundsException () {
		try {
			...
			System.out.println("Error de �ndice de acceso a un elemento en un array");
			System.out.println("Mensaje de error: " + ex.getMessage() + "\n\n");
			ex.printStackTrace();
		}
	}
	...
	private static void ejemploNumberFormatException () {
		try {
			...
			System.out.println("Error de parseo a un n�mero");
			System.out.println("Mensaje de error: " + ex.getMessage() + "\n\n");
			ex.printStackTrace();
		}
	}
	...
	private static void ejemploClassNotFoundException () {
		try {
			...
			System.out.println("\nListado de m�todos del objeto: ");
			for (int i = 0; i < metodos.length; i++) {
				System.out.println("M�todo: " + metodos[i].getName() + "()");
			}
			
			// Ejemplo de invocaci�n de un m�todo de un objeto
			System.out.println("\nInvocaci�n pierna derecha: " + Persona.class.getMethod("getPiernaDerecha", null).invoke(persona, null));
			...
			System.out.println("IllegalArgumentException Error en la invocaci�n de un m�todo");
			System.out.println("Mensaje de error: " + e.getMessage() + "\n\n");
			e.printStackTrace();
		} catch (InvocationTargetException e) {
			System.out.println("InvocationTargetException Error en la invocaci�n de un m�todo");
			System.out.println("Mensaje de error: " + e.getMessage() + "\n\n");
			e.printStackTrace();
		} catch (NoSuchMethodException e) {
			System.out.println("NoSuchMethodException Error en la invocaci�n de un m�todo");
			System.out.println("Mensaje de error: " + e.getMessage() + "\n\n");
			e.printStackTrace();
		} catch (SecurityException e) {
			System.out.println("SecurityException Error en la invocaci�n de un m�todo");
			System.out.println("Mensaje de error: " + e.getMessage() + "\n\n");
			e.printStackTrace();
		} catch(Exception ex) {
